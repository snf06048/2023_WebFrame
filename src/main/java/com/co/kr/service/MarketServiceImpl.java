...
import com.co.kr.mapper.MarketMapper;
...
public class MarketServiceImpl implements MarketService {
	...
	public int fileProcess(MarketListVO marketListVO, MultipartHttpServletRequest request, HttpServletRequest httpReq) {
		...
		//content domain 생성 
		MarketContentDomain marketContentDomain = MarketContentDomain.builder()
				.mbId(session.getAttribute("id").toString())
				.mkTitle(marketListVO.getTitle())
				.mkContent(marketListVO.getContent())
				.build();
		
				if(marketListVO.getIsEdit() != null) {
					marketContentDomain.setMkSeq(Integer.parseInt(marketListVO.getSeq()));
					System.out.println("수정업데이트");
					// db 업데이트
					marketMapper.mkContentUpdate(marketContentDomain);
				}else {	
					// db 인서트
					marketMapper.contentUpload(marketContentDomain);
					System.out.println(" db 인서트");

				}
				
				// file 데이터 db 저장시 쓰일 값 추출
				int mkSeq = marketContentDomain.getMkSeq();
				String mbId = marketContentDomain.getMbId();
				...
				if(marketListVO.getIsEdit() != null) { // 수정시 
				...
				// 저장 root 경로만들기
				Path rootPath = Paths.get(new File("/Users/kimhyeonseob/").toString(),"mkupload", File.separator).toAbsolutePath().normalize();			
				...
				for (MultipartFile multipartFile : multipartFiles) {
					...
					if(!multipartFile.isEmpty()) {  // 파일 있을때 
						...
						try {
							...
							//파일 domain 생성 
							MarketFileDomain marketFileDomain = MarketFileDomain.builder()
									.mkSeq(mkSeq)
									.mbId(mbId)
									.upOriginalFileName(origFilename)
									.upNewFileName("resources/mkupload/"+newFileName) // WebConfig에 동적 이미지 폴더 생성 했기때문
									.upFilePath(targetPath.toString())
									.upFileSize((int)multipartFile.getSize())
									.build();
							
								// db 인서트
								marketMapper.fileUpload(marketFileDomain);
